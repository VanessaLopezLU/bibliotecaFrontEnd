...
export default {
    ...
    methods: {
        ...
        async cargar() {
            var vm = this
            await axios
                .get("http://localhost:3000/estado-usuario/obtener")
                .then(function (response) {
                    // handle success
        ...
        editItem(item) {
         ...
         async deleteItem(id) {
            alert(id);
            await axios.delete('http://localhost:3000/estado-usuario/eliminarestado/' + id).then(response => {
        ...
        async editarEstado() {
            try {
                await axios.put('http://localhost:3000/estado-usuario/actualizarestado',this.paqueteEditar).then(() => {
